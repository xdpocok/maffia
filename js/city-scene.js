...
class CityScene extends Phaser.Scene {
  ...
  refreshHUD() {
    ...
    const influence = typeof ensureInfluenceState === "function"
      ? ensureInfluenceState()
      : Math.max(0, Math.min(100, Math.round(Number(state.influence) || 10)));
    if (hudInfluence) hudInfluence.textContent = `${influence}%`;
    if (typeof renderInfluenceInfo === "function" && hudInfluencePill?.getAttribute("aria-expanded") === "true") {
      renderInfluenceInfo();
    }
