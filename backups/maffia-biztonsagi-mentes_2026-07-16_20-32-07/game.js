...
function getGarageRoundSafeWidth(gameState, mission) {
  ...
  return clamp(baseWidth - clickCount * 0.018, 0.1, 0.34);
}
...
function getGarageAttemptSpeed(gameState, mission) {
  ...
  const clickCount = Math.max(0, Math.round(Number(gameState?.attempts) || 0));
  return clamp(baseSpeed + clickCount * 0.002, 0.016, 0.05);
}
...
function mergeServerRobberyAction(action, spot = null) {
  ...
  return {
    ...
    defenders: Array.isArray(action.defenders) ? action.defenders : [],
    allies: Array.isArray(action.allies) ? action.allies : [],
    selectedMemberIds: Array.isArray(action.selectedMemberIds) ? action.selectedMemberIds : [],
    selectedDefenderId: (action.defenders || []).find((entry) => entry.health > 0)?.id || null,
...
function showServerRobberyResult(action) {
  ...
  showRobberyResult(success, success ? "Sikeres rajtautes" : "A rajtautes kudarcba fulladt", text);
  sceneRef?.refreshHUD();
  sceneRef?.refreshMap();
}
...
playRobberyTurn = async function playThreeVsThreeTurn() {
  ...
  try {
    ...
    if (activeRobberyGame.autoPlay) queueRobberyAutoPlay(activeRobberyGame, 460);
  } catch (error) {
