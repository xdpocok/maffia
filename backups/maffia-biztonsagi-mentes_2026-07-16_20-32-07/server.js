...
const ROBBERY_TACTICS = {
  stealth: { name: "Lopakodas", strongAgainst: "watcher", damage: 1.04, alert: 3 },
  force: { name: "Fegyveres roham", strongAgainst: "bodyguard", damage: 1.13, alert: 8 },
  intimidation: { name: "Megfelemlites", strongAgainst: "boss", damage: 1.08, alert: 5 },
};
...
function normalizeRobberyTarget(body = {}, state = {}) {
  ...
  const difficulty = Math.round(security * 0.45 + variance * 42 + (mode === "shop" ? 10 : -4));
  return {
    spotId,
    name: String(body.name || (mode === "shop" ? "Uzlet" : "Utcai celpont")).trim().slice(0, 80),
    mode,
    districtIndex,
    difficulty: clampServer(difficulty, 12, 95),
    cycle,
  };
}
...
function getServerRobberyDifficultyInfo(state, difficulty) {
  ...
  const difference = difficulty - Math.max(1, combat.assault);
  if (difference <= 5) return { label: "Konnyu", successChance: clampServer(0.86 - difference * 0.012, 0.78, 0.96) };
  if (difference <= 20) return { label: "Kockazatos", successChance: clampServer(0.68 - difference * 0.018, 0.38, 0.7) };
  return { label: "Veszelyes", successChance: clampServer(0.42 - difference * 0.012, 0.12, 0.38) };
}
...
function createServerRobberyAllies(profileName, state, selectedMemberIds = []) {
  ...
  const members = getOwnedCrewMembers(state)
    .filter((member) => selected.has(String(member.id || "")) && Number(member.health) > 0)
    .slice(0, 2)
    .map((member) => {
      ...
      return {
        ...
        level: stats.level,
        maxHealth,
        health: clampServer(member.health, 0, maxHealth),
        attack: stats.attack,
  ...
  const player = {
    ...
    attack: Math.max(4, combat.playerAttack),
    defense: Math.max(3, combat.playerDefense),
    isPlayer: true,
  };
...
function createServerRobberyDefenders(action, allies) {
  ...
  const count = clampServer(allies.length, 1, 3);
  const averageAttack = allies.reduce((sum, ally) => sum + ally.attack, 0) / allies.length;
  const averageDefense = allies.reduce((sum, ally) => sum + ally.defense, 0) / allies.length;
  const averageHealth = allies.reduce((sum, ally) => sum + ally.maxHealth, 0) / allies.length;
  const averageLevel = allies.reduce((sum, ally) => sum + ally.level, 0) / allies.length;
  const tier = action.difficultyInfo.label === "Veszelyes"
    ? { attack: 1.18, defense: 1.14, health: 1.16, damage: 1.14 }
    : action.difficultyInfo.label === "Kockazatos"
      ? { attack: 0.98, defense: 0.96, health: 0.98, damage: 1 }
      : { attack: 0.76, defense: 0.74, health: 0.78, damage: 0.82 };
  return names.slice(0, count).map((name, index) => {
    const roleScale = types[index] === "boss" ? 1.07 : types[index] === "bodyguard" ? 1.03 : 0.94;
    const variance = 0.96 + hashStringUnit(`${action.actionId}:${index}`) * 0.08;
    const level = clampServer(Math.round(averageLevel + index * 0.5), 1, 30);
    const maxHealth = Math.max(24, Math.round(averageHealth * tier.health * roleScale * variance));
    return {
      ...
      maxHealth,
      health: maxHealth,
      attack: Math.max(4, Math.round(averageAttack * tier.attack * roleScale * variance)),
      defense: Math.max(3, Math.round(averageDefense * tier.defense * roleScale * variance)),
...
function syncRobberyHealthToState(state, action) {
  ...
  state.crewMembers = state.crewMembers.map((member) => {
    ...
    return ally ? { ...member, health: clampServer(ally.health, 0, Math.max(1, Number(member.baseHealth) || 100)) } : member;
  });
...
function advanceServerRobberyQuests(state, mode) {
  ...
  state.activeQuests = state.activeQuests.map((quest) => {
    if (quest?.status !== "accepted" || quest?.goal?.action !== "robbery") return quest;
    if (quest.goal.mode !== "any" && quest.goal.mode !== mode) return quest;
    const target = Math.max(1, toSafeInt(quest.goal.target, 1, 1));
    const progress = clampServer(toSafeInt(quest.goal.progress, 0, 0) + 1, 0, target);
...
function normalizeServerCrewMembers(state = {}) {
  ...
  return SERVER_CREW_TEMPLATES.map((template) => {
    ...
    return {
      ...
      health: clampServer(saved.health ?? template.baseHealth, 0, template.baseHealth),
...
async function runCrewEconomyCommand(profileName, body = {}) {
  return db.transaction(async () => {
    ...
    if (operation === "hire") {
      ...
      state.money -= cost;
      gainedPoints = randomServerInt(1, 3);
      member.level += 1;
...
async function runMarketBuyCommand(profileName, body = {}) {
  return db.transaction(async () => {
    ...
    const price = Math.max(0, toSafeInt(marketRow.price, 0, 0));
    ...
    await persistPvpState(profileName, state, now);
    await logEvent(profileName, "market_purchase", "Feketepiaci vasarlas", { itemId, slot, price }, now);
    return { statusCode: 200, payload: { ok: true, item, price, state: buildEconomyClientState(state) } };
  });
}
...
function normalizeServerQuest(quest, allowedStatuses = ["offered", "accepted", "completed"]) {
  ...
  const action = quest.goal?.action === "protection" || quest.type === "protection" ? "protection" : "robbery";
  const mode = ["shop", "street"].includes(quest.goal?.mode) ? quest.goal.mode : "any";
  ...
  return {
    ...
    type: action === "protection" ? "protection" : "robbery",
    status: quest.status,
    ...
    goal: { action, mode, target, progress },
...
async function runQuestProgressionCommand(profileName, body = {}) {
  return db.transaction(async () => {
    ...
    if (operation === "accept") {
      ...
      quest = { ...state.offeredQuests[offeredIndex], status: "accepted", goal: { ...state.offeredQuests[offeredIndex].goal, progress: 0 } };
...
function applyServerPoliceBust(state) {
  ...
  const moneyLoss = Math.floor(Math.max(0, toSafeInt(state.money, 0, 0)) * 0.22);
  state.money = Math.max(0, state.money - moneyLoss);
  state.heat = clampServer(state.heat - 15, 0, 100);
  state.crew = normalizeServerCrewMembers(state).filter((member) => member.hired).length;
  return { moneyLoss, heatLoss: 15 };
}
...
async function runEmpireProgressionCommand(profileName, body = {}) {
  return db.transaction(async () => {
    ...
    if (operation === "city-upgrade") {
      ...
      result = { districtIndex, districtName: district.name || district.id, healthLoss, fameGain: 12, heatGain: 8, bust };
      ...
      result = { income, districtIncome, territoryIncome, fameGain, controlledCount: controlled.length, day: state.day };
    } else {
...
function advanceServerActionQuests(state, actionType, mode) {
  if (!Array.isArray(state.activeQuests)) return;
  state.activeQuests = state.activeQuests.map((quest) => {
    if (quest?.status !== "accepted" || quest?.goal?.action !== actionType) return quest;
    if (quest.goal.mode !== "any" && quest.goal.mode !== mode) return quest;
    const target = Math.max(1, toSafeInt(quest.goal.target, 1, 1));
    const progress = clampServer(toSafeInt(quest.goal.progress, 0, 0) + 1, 0, target);
    return { ...quest, status: progress >= target ? "completed" : quest.status, goal: { ...quest.goal, progress } };
...
async function runRecoveryProgressionCommand(profileName, body = {}) {
  return db.transaction(async () => {
    ...
    if (operation === "start") {
      ...
      if (body.layLow) {
        const day = Math.max(1, toSafeInt(state.day, 1, 1));
        if (toSafeInt(state.hideUsesDay, day, 1) !== day) {
          state.hideUsesDay = day;
          state.hideUsesToday = 0;
        }
        if (toSafeInt(state.hideUsesToday, 0, 0) >= 3) return { statusCode: 409, error: "A Lapulast ma mar haromszor hasznaltad." };
        state.hideUsesToday = toSafeInt(state.hideUsesToday, 0, 0) + 1;
      ...
      await logEvent(profileName, `recovery_${stat}_started`, stat === "health" ? "Lapulas elinditva" : "Talalkozo elinditva", { layLow: Boolean(body.layLow), heatLoss }, now);
    }
    await persistPvpState(profileName, state, now);
    return { statusCode: 200, payload: { ok: true, operation, heatLoss, state: buildProgressionClientState(state) } };
  });
}
...
async function runProtectionProgressionCommand(profileName, body = {}) {
  return db.transaction(async () => {
    ...
    const combat = getPvpCombatStats(state);
    const adjustedChance = clampServer(difficultyInfo.successChance * (0.74 + combat.readiness * 0.2), 0.18, 0.88);
    const gainBase = 14 + Math.round(target.difficulty * 0.16) + Math.max(1, toSafeInt(state.cityLevel, 1, 1)) * 4 + randomServerInt(0, 14);
    const readinessPenalty = combat.readiness < 0.6 ? Math.round((0.6 - combat.readiness) * 16) : 0;
    const gain = Math.max(10, Math.round(gainBase * (0.84 + difficultyInfo.successChance * 0.42) - readinessPenalty));
    ...
    await logEvent(profileName, success ? "protection_success" : "protection_failed", success ? "Vedelmi penz befolyt" : "Vedelmi penz beszedese sikertelen", {
      spotId: target.spotId, success, moneyGain, fameGain: success ? fameGain : 0, healthLoss, heatGain, successChance: adjustedChance,
    }, now);
    return {
      statusCode: 200,
      payload: {
        ok: true,
        result: { success, buildingName: target.name, moneyGain, fameGain: success ? fameGain : 0, healthLoss, heatGain, successChance: adjustedChance },
...
async function startServerRobbery(profileName, body) {
  return db.transaction(async () => {
    ...
    if (activeAction && Number(activeRow.expires_at) > now) {
      return {
        statusCode: 200,
        payload: {
          ok: true,
    ...
    if (activeAction) {
      activeAction.status = "expired";
      activeAction.result = { success: false, reason: "Az akcio ideje lejart." };
    ...
    const action = {
      ...
      status: "selection",
      target,
      difficultyInfo: getServerRobberyDifficultyInfo(state, target.difficulty),
      energyCost,
      battleStarted: false,
      battleMode: "full",
      rewardMultiplier: 1,
      ...
      loot: 0,
      alert: 0,
      message: "Valassz legfeljebb ket harckepes bandatagot.",
      createdAt: now,
...
async function engageServerRobbery(profileName, actionId, body) {
  return db.transaction(async () => {
    ...
    action.message = action.battleMode === "lone"
      ? "Egyedul indultal. A jutalom csokkentett."
      : action.battleMode === "solo"
...
function finishServerRobberyState(profileName, state, action, outcome, reason, now) {
  ...
  if (won) {
    ...
    fameGain = action.target.mode === "shop" ? 8 : 5;
    state.money = Math.max(0, toSafeInt(state.money, 0, 0) + moneyGain);
    state.fame = Math.max(0, toSafeInt(state.fame, 0, 0) + fameGain);
  ...
  action.battleStarted = true;
  action.result = { success: won, reason, moneyGain, fameGain, heatGain };
...
async function playServerRobberyTurn(profileName, actionId, body) {
  return db.transaction(async () => {
    ...
    const strongBonus = tactic.strongAgainst === target.type ? 1.16 : 1;
    const damage = clampServer(
      Math.round((attack + attacker.level + randomServerInt(-4, 5)) * tactic.damage * strongBonus - defense * 0.58),
      3,
      Math.max(5, Math.round(attack * 1.05)),
    );
    ...
    action.alert = clampServer(action.alert + tactic.alert + randomServerInt(1, 4), 0, 100);
    action.message = `${attacker.name} ${damage} sebzest okozott ${target.name} ellen.`;
    ...
    if (enemiesAfterAttack.length) {
      ...
      const allyTarget = targets[randomServerInt(0, targets.length - 1)];
      ...
      const retaliation = clampServer(
        Math.round((enemyAttack + randomServerInt(-3, 4) - allyDefense * 0.48) * (enemy.damageScale || 1)),
        2,
        Math.max(6, Math.round(enemyAttack * 0.9)),
      );
      allyTarget.health = clampServer(allyTarget.health - retaliation, 0, allyTarget.maxHealth);
      action.message += ` ${enemy.name} visszavagott ${allyTarget.name} ellen: -${retaliation} HP.`;
    }
...
async function handleApiRequest(request, response, pathname) {
  ...
  if (progressionActionMatch && request.method === "POST") {
    try {
      ...
      sendJson(response, result.statusCode, result.payload || { error: result.error, cooldownAt: result.cooldownAt });
      return true;
    } catch (error) {
  ...
  const economyActionMatch = pathname.match(/^\/api\/actions\/economy\/(crew|equip|market-buy|craft)$/);
