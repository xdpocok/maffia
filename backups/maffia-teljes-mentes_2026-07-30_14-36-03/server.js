  ...
  main_quest_templates: {
    ...
    payload: {
      ...
      standard: [
        ...
        { type: "cargo_spend", title: "Titkos atadas", description: "Adj le {target} darab {cargo} arut kikotoi megbizasok inditasaval.", objective: "Hasznalj fel {target} darab {cargo} arut a Kikoto negyedben.", goal: { action: "cargo_spend", mode: "randomCargo", targetMin: 2, targetMax: 7, progress: 0 }, xp: 42, money: 225 },
...
function normalizeRobberyTarget(body = {}, state = {}) {
  ...
  const generatedTierOffset = variance < 0.56
    ? -3 - Math.round(variance * 5)
    : variance < 0.84
      ? 8 + Math.round(variance * 11)
      : 24 + Math.round(variance * 16);
  const generatedDifficulty = Math.max(
    1,
    Math.round(combat.assault + generatedTierOffset + (mode === "shop" ? 2 : 0) + (security - 50) * 0.03),
  );
...
function createServerRobberyAllies(profileName, state, selectedMemberIds = []) {
  ...
  const members = getOwnedCrewMembers(state)
    ...
    .map((member) => {
      ...
      const storedMaxHealth = Math.max(1, toSafeInt(member.baseHealth, 100, 1));
      const healthPercent = clampServer(Number(member.health) / storedMaxHealth, 0, 1);
      const maxHealth = 100;
      return {
        id: String(member.id),
        ...
        level: stats.level,
        maxHealth,
        health: Math.round(healthPercent * maxHealth),
        attack: stats.attack,
...
function getServerRobberyTier(label) {
  if (label === "Veszelyes") return { attack: 1.28, defense: 1.2, damage: 1.24 };
  if (label === "Kockazatos") return { attack: 1.08, defense: 1.04, damage: 1.09 };
  return { attack: 0.88, defense: 0.86, damage: 0.9 };
}
...
function applyServerHarborReward(state, mission) {
  ...
  const result = { missionId: mission.id, title: mission.title, success, moneyGain: 0, xpGain: 0, fine: 0, cargo: {}, heal: 0, energy: 0 };
  if (!success) {
    result.fine = Math.min(state.money, Math.max(12, Math.round(Number(mission.rewardMoney || 0) * 0.18)));
...
async function runHarborCommand(profileName, body = {}) {
  return db.transaction(async () => {
    ...
    if (operation === "start") {
      ...
      for (const [key, amount] of Object.entries(requires)) {
      ...
      const task = {
        ...
        payload: { missionId: mission.id, title: mission.title, zone: mission.zone },
      };
    ...
    if (operation === "upgrade") {
      ...
      if (action.round > action.totalRounds) {
        ...
        if (success) {
          ...
          result = { success, moneyGain: Math.max(20, Math.round(action.rewardMoney * factor)), xpGain: Math.max(6, Math.round(action.rewardXp * factor)), cargo: {} };
          ...
          result = { success, penalty: Math.min(state.money, mission.failurePenalty), heatGain: mission.heatFail };
...
async function runProtectionProgressionCommand(profileName, body = {}) {
  return db.transaction(async () => {
    ...
    const adjustedChance = clampServer(protectionBaseChance - readinessPenalty, 0.2, 0.96);
...
async function ensureDefaultGameConfigEntries() {
  ...
  const questConfigVersion = "mixed-main-harbor-quests-v1";
...
async function engageServerRobbery(profileName, actionId, body) {
  return db.transaction(async () => {
    ...
    const readyMemberIds = new Set(getOwnedCrewMembers(profile.state)
    ...
    if (readyMemberIds.size > 0 && selectedMemberIds.length === 0) {
      return { statusCode: 400, error: "Valassz legalabb egy harckepes bandatagot a harc elott." };
    }
...
async function playServerRobberyTurn(profileName, actionId, body) {
  return db.transaction(async () => {
    ...
    if (correctedDifficultyInfo.label !== action.difficultyInfo.label) {
    ...
    const damage = clampServer(
      Math.round(6 + attack * 0.16 + attacker.level * 0.12 + randomServerInt(-2, 2) - defense * 0.08)
        * tactic.damage * strongBonus,
      5,
      16,
    );
    ...
    if (enemiesAfterAttack.length) {
      ...
      const allyTarget = Math.random() < focusChance
        ? weakestTarget
      ...
      const minimumRetaliation = difficultyLabel === "Veszelyes" ? 6 : difficultyLabel === "Kockazatos" ? 5 : 4;
      const maximumRetaliation = difficultyLabel === "Veszelyes" ? 19 : difficultyLabel === "Kockazatos" ? 16 : 13;
      const retaliation = clampServer(
        Math.round((5 + enemyAttack * 0.17 + enemy.level * 0.12 + randomServerInt(-2, 2) - allyDefense * 0.055)
          * (enemy.damageScale || 1)),
        minimumRetaliation,
        maximumRetaliation,
      );
