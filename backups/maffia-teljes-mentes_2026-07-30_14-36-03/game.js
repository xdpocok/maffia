...
function createRandomBuildingDifficulties(cycle = getBuildingDifficultyCycle(), profileSeed = state.profileName || "") {
  ...
  orderedSpots.forEach((spot, index) => {
    ...
    if (index < MIN_DANGER_BUILDINGS) {
      difficulties[spot.id] = playerPower + 24 + Math.round(baseRoll * 16);
    } else if (index < MIN_DANGER_BUILDINGS + MIN_RISK_BUILDINGS) {
      difficulties[spot.id] = playerPower + 8 + Math.round(baseRoll * 11);
    } else {
      difficulties[spot.id] = Math.max(1, playerPower - 3 - Math.round(baseRoll * 5));
    }
  });
  return difficulties;
}
...
function finishGarageMissionFromServer(result = {}) {
  ...
  if (result.success) {
    ...
    sceneRef?.pushLog(`${mission.title} megbukott. -${penalty} $.`);
...
const mainQuestTemplateDefs = {
  ...
  standard: [
    ...
    },
    {
      type: "cargo_spend",
      title: "Titkos atadas",
      description: "Adj le {target} darab {cargo} arut kikotoi megbizasok inditasaval.",
      objective: "Hasznalj fel {target} darab {cargo} arut a Kikoto negyedben.",
...
const proceduralQuestBlueprints = [
  { id: "rob-shop", group: "main", action: "robbery", mode: "shop", min: 1, max: 4, money: 72, xp: 13, label: (target) => `Rabolj ki ${target} boltot a fotérképen.` },
  ...
  { id: "cargo-sell", group: "harbor", action: "cargo_spend", mode: "randomSpendCargo", min: 1, max: 8, money: 54, xp: 14, label: (target, cargo) => `Adj le ${target} darab ${cargo} arut kikotoi megbizassal.` },
  ...
  for (let attempt = 0; attempt < 30; attempt += 1) {
    ...
    return {
      ...
      description: groups.size > 1
        ? "Osszetett csaladi megbizas: dolgozz a fotérképen, majd zard le a kikotoi reszt is."
        : groups.has("harbor")
...
function renderProcessTasks(now = Date.now()) {
  ...
  hudProcessTasks.innerHTML = `
    <div class="hud-process-group hud-process-group--${kind}">
      <b><svg class="hud-process-anchor" viewBox="0 0 34 34" aria-hidden="true"><circle cx="17" cy="5" r="3"/><path d="M17 8v18M11 12h12M14 15h6M7 17c0 8 3.5 12 10 15 6.5-3 10-7 10-15M3 21l4-4 4 5M31 21l-4-4-4 5M13 10c-3 2-4 5-4 8M21 10c3 2 4 5 4 8"/></svg><span>${escapeHtml(getProcessTaskLabel(kind))}</span></b>
...
function showCompletedHarborResult(result = {}) {
  if (result.success) {
    sceneRef?.pushLog(`${result.title || "Kikotoi munka"} kesz. +${result.moneyGain || 0} $, +${result.xpGain || 0} XP.`);
    ...
    sceneRef?.pushLog(`${result.title || "Kikotoi munka"} elbukott. -${result.fine || 0} $ buntetes.`);
...
async function startRecovery(stat, options = {}) {
  ...
  try {
    await requestServerProgression("recovery", { operation: "start", stat, layLow: Boolean(options.layLow) });
    sceneRef?.refreshHUD();
    sceneRef?.refreshMap();
    return true;
  } catch (error) {
    sceneRef?.setMessage(error.message || "A toltes nem indithato el.");
    return false;
  }
}
...
function getBandPowerProfile() {
  ...
  const getAdjustedAttack = (member) => getCrewMemberAttack(member) + Math.floor((member?.level || 1) * 0.45);
  const getAdjustedDefense = (member) => getCrewMemberDefense(member) + Math.floor((member?.defenseLevel || member?.level || 1) * 0.4);
  const crewAttackTotal = members.reduce((sum, member) => sum + getAdjustedAttack(member), 0);
  const crewDefenseTotal = members.reduce((sum, member) => sum + getAdjustedDefense(member), 0);
  ...
  const crewReadinessAverage = members.length
    ...
    : 1;
  const activeAttack = activeMember ? getAdjustedAttack(activeMember) : 0;
  const activeDefense = activeMember ? getAdjustedDefense(activeMember) : 0;
...
async function runTerritoryAction(actionId, territory) {
  if (territory.kind === "park") {
    if (actionId === "robbery") {
      ...
      if (!await startRecovery("health", { layLow: true })) return;
...
function getDifficultyInfo(difficulty, actionType = "robbery") {
  ...
  const difference = difficulty - actionPower;
...
async function collectProtectionMoney(targetDistrict, buildingName = "Haz", targetSpot = null) {
  ...
  try {
    ...
    if (result.success) {
      completeMentorStep("protection");
      sceneRef?.pushLog(`${buildingName}: vedelmi penz befolyt. +${result.moneyGain} $, +${result.fameGain} XP, +${result.heatGain}% korozes.`);
...
async function handleExpand() {
  ...
  try {
    ...
    const result = response.result;
    sceneRef?.pushLog(`${target.name} a birodalom resze lett. -${result.healthLoss} eletero.`);
    if (result.bust) {
      sceneRef?.pushLog(`Rendőrségi rajtaütés: -${result.bust.moneyLoss} $, -${result.bust.heatLoss}% körözés.`);
    }
...
async function endDay() {
  ...
  try {
    ...
    const result = response.result;
    sceneRef?.pushLog(`Nap vege: +${result.income} $ (${result.territoryIncome} $ hazbevetel), +${result.fameGain} hirnev.`);
    sceneRef?.refreshHUD();
    sceneRef?.refreshMap();
...
function ensureCityEngine() {
  ...
  const featureStyles = Promise.all([
    loadStylesheet("./styles/combat.css?v=perf-2026-07-19-2"),
    loadStylesheet("./styles/features.css?v=perf-2026-07-19-2"),
  ]).then(() => loadStylesheet("./styles/hud-redesign.css?v=map-balance-2026-07-28-12"));
  ...
  cityEnginePromise = Promise.all([featureStyles, phaserEngine])
    .then(() => loadClassicScript("./js/city-scene.js?v=perf-2026-07-19-2"))
...
function canStartRobberySelection(encounter) {
  if (!encounter || encounter.battleStarted) return false;
  const hasReadyMember = getHiredCrewMembers().some((member) => Number(member.health) > 0);
  return !hasReadyMember || encounter.selectedMemberIds.length > 0;
}
...
async function requestServerEconomy(command, payload = {}) {
  ...
  if (!response.ok) throw new Error(data.error || "A szerver nem tudta vegrehajtani a muveletet.");
...
function mergeServerRobberyAction(action, spot = null) {
  ...
  return {
    ...
    healthAtStart: existing.healthAtStart ?? state.health,
...
function showServerRobberyResult(action) {
  ...
  const text = success
    ? `Zsakmany: +${result.moneyGain || 0} $ · Hirnev: +${result.fameGain || 0} · Korozes: +${result.heatGain || 0}%`
    : `${result.reason || "A rajtautes kudarcba fulladt."} Korozes: +${result.heatGain || 0}%`;
  if (success) {
    completeMentorStep("robbery");
    queueRewardModal({
      title: "Akcio vege",
      text: activeRobberyGame.spot?.name || "Kirablas",
  ...
  sceneRef?.pushLog(success
    ? `${activeRobberyGame.spot?.name || "A celpont"} kirabolva. A jutalmat a szerver irta jova.`
    : result.reason || "A kirablas kudarcba fulladt.");
